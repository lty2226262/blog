...
public class SimpleBoard {
  ...
  public boolean equals(Object board) {
    ...
    if(board instanceof SimpleBoard){
      SimpleBoard toCompare = (SimpleBoard)board;
      for(int i = 0; i < DIMENSION;i++){
        for(int j = 0; j < DIMENSION;j++){
          if(grid[i][j] != toCompare.grid[i][j]) return false;
        }
      }
      return true;
    }
  ...
  public int hashCode() {
    ...
    for(int i = 0; i < DIMENSION;i++){
      for(int j = 0; j < DIMENSION;j++){
        ans = (ans.multiply(BigInteger.valueOf(3))).add(BigInteger.valueOf(grid[i][j]));
      }
    }
    return ans.hashCode();
  }
