...
class robot:
    ...
    def cte(self, radius):
        ...
        if c_x < radius:
            cte = sqrt((c_x - radius)**2 + (c_y - radius) ** 2) - radius
        elif c_x >= radius and c_x <= 3.0 * radius:
            if self.orientation >= 0.5 * pi and self.orientation <= 1.5 * pi:
                cte = -c_y
            else:
                cte = c_y - 2.0 * radius
        else:
            cte = sqrt((c_x - 3.0 * radius) ** 2 + (c_y - radius) ** 2) - radius
...
def run(params, radius, printflag=False):
    ...
    for i in range(N * 2):
        ...
        if printflag:
            print myrobot
        x_set.append(myrobot.x)
        y_set.append(myrobot.y)
        o_set.append(myrobot.orientation)
    return err / float(N), x_set, y_set, o_set


radius = 25.0
params = [10.0, 15.0, 0]
err, x_set, y_set, o_set = run(params, radius, True)
print '\nFinal parameters: ', params, '\n ->', err

DATA = np.random.rand(len(x_set),len(x_set))
...
for i in range(len(x_set)):
    colorVal = scalarMap.to_rgba(DATA[i, 4])
    plt.quiver(x_set[i], y_set[i], 9.0 * cos(o_set[i]), 9.0 * sin(o_set[i]), scale_units='xy', angles='xy', scale=0.5,color=colorVal)
    print x_set[i]+sin(o_set[i])
plt.show()
