...
class robot:
    ...
    def move(self,motion):
        ...
        motion[0] = motion[0] + random.gauss(0.0, self.steering_noise)
        motion[1] = motion[1] + random.gauss(0.0, self.distance_noise)

        if (fabs(beta) >= 0.001):

            R = motion[1] / beta

            c_x = self.x - sin(self.orientation) * R
            c_y = self.y - cos(self.orientation) * R

            x1_ = c_x + sin(self.orientation + beta) * R
            y1_ = c_y + cos(self.orientation + beta) * R

            theta_ = (self.orientation + beta) % (2 * pi)

        else:
            x1_ = self.x + motion[1] * cos(self.orientation)
            y1_ = self.y + motion[1] * sin(self.orientation)
    ...
    def sense(self, add_noise = 1):
        ...
        for i in landmarks:
            ...
            p_angle = atan2(d_y, d_x)
            p_angle = (p_angle - self.orientation)
            if (add_noise):
                p_angle +=  random.gauss(0.0, self.bearing_noise)
            p_angle %=  (2 * pi)
            # ENTER CODE HERE
